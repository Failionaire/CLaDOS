...
Gate actions:
- **Approve →** — primary action, proceeds to next phase; blocked if any `must_fix` findings remain unresolved; shows a confirmation dialog if `should_fix` or suggestion findings are unaddressed
...
After Gate 1 is approved (concept accepted), the topbar displays a **GLaDOS progress bar** — a cost-weighted dual-rail bar spanning all 5 phases. Each segment's width is proportional to √cost (estimated before actual data, then updated from real spend), so the Build phase visibly dominates without swallowing the bar. The active segment glows orange; each phase has a snarky sublabel that changes with state (e.g. *"overthinking the schema."* while running, *"overengineered, naturally."* when done). The bar also shows the active agent name and a blinking cursor while running, and the gate number when awaiting review. A **running cost total** (*"$0.38 used"*) sits in the topbar alongside the bar, updated after each API call; hovering shows a per-phase breakdown of actual spend. No upfront total estimate is displayed — lower-bound estimates anchor user expectations and erode trust when actual costs are materially higher due to revisions and escalations.
